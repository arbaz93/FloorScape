...
const createProject = async ({ item, visibility = "private" }:CreateProjectParams): Promise<DesignItem | null | undefined> => {
    if(!PUTER_WORKER_URL) {
    ...
    if(!resolvedSource) {
        console.warn("failed to save source image ...");
        return null
    }
...
export { signIn, signOut, getCurrentUser, createProject, getProjects, getProjectById }